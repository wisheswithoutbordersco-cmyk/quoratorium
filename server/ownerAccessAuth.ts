...
function configuredCode(): string {
  return (
    ...
    process.env.BUSINESS_ACTION_PIN ||
    ""
  ).trim();
}

function signingSecret(): string {
  return process.env.OWNER_ACCESS_SESSION_SECRET || "";
}
...
export function isOwnerAccessConfigured(): boolean {
  ...
  return (
    configuredCode().length >= 8 &&
    signingSecret().length >= minimumSecretLength
  );
}
...
export function verifyOwnerAccessCode(
  ...
  const ok = timingSafeEqual(
    codeDigest(suppliedCode.trim()),
    codeDigest(configuredCode())
  );
