...
beforeEach(() => {
  ...
  process.env.OWNER_ACCESS_SESSION_SECRET = "test-owner-session-secret-with-entropy";
});
...
describe("owner access authentication", () => {
  it("verifies the configured access code without exposing it", () => {
    ...
    expect(verifyOwnerAccessCode(request(), "owner-access-47")).toEqual({ ok: true });
    expect(verifyOwnerAccessCode(request(), "wrong-access-code")).toEqual({ ok: false });
  });
  ...
  it("creates a signed HTTP-only session and rejects tampering", () => {
    ...
    expect(getOwnerAccessSession(request(cookie))).toMatchObject({ ownerId: 42 });
  ...
  it("requires an independent high-entropy signing secret in production", () => {
    const keys = [
      "OWNER_ACCESS_SESSION_SECRET",
      "BUSINESS_ACTION_SESSION_SECRET",
      "BUSINESS_CREDENTIAL_KEY",
      "CLERK_SECRET_KEY",
      "NODE_ENV",
    ] as const;
    ...
    try {
      ...
      expect(verifyOwnerAccessCode(request(), "owner-access-47")).toEqual({ ok: false });
      expect(() => startOwnerAccessSession({ cookie: vi.fn() } as any, 42)).toThrow(
        "Owner access session signing is not configured",
      );
    } finally {
