...
const plan: RecyclatoriumPlan = {
  ...
  visualSummary: "The sources show illustrated country covers with children, landmarks, plants, animals, and regional design details.",
  whyTogether: "The shared day-in-the-life structure supports careful observation, respectful comparison, vocabulary, and creative response activities.",
  teachingGoals: ["Notice visual details", "Build descriptive vocabulary", "Compare places respectfully"],
  palette: ["#E27632", "#2B8A78", "#F2C14E"],
  activities: Array.from({ length: 4 }, (_, index) => ({
    title: `Visual Detective ${index + 1}`,
    instructions: "Study the source spark and circle details that reveal place, weather, food, clothing, or landscape.",
    ...
    prompt: "I notice ______. This detail makes me think ______ because ______.",
    extension: "Draw one new detail that would fit the setting and explain your choice.",
  })),
  listing: {
    shortDescription: "A printable visual-literacy pack for exploring world cultures.",
    longDescription: "Students use illustrated country scenes to observe, compare, write, and create through four original classroom activities.",
    tags: ["world cultures", "visual literacy", "social studies", "elementary", "printable"],
    suggestedPrice: "$5.99",
  },
};
...
describe("Recyclatorium product generation", () => {
  it("parses a strict product plan and rejects non-JSON fallback text", () => {
    ...
    expect(() => parseRecyclatoriumPlan("Falling back to filename heuristics")).toThrow(/malformed JSON/i);
  });
  ...
  it("keeps exported filenames and PDF text portable", () => {
    expect(safeProductFilename("World Cultures — PreK–3!")).toBe("World-Cultures-PreK-3");
    expect(sanitizePdfText("It’s a “new” pack…")).toBe("It's a \"new\" pack...");
    expect(readableSourceName("220a32ed-f633-4e80-a3dd-cc4b11edf86c4284312017593918808.jpg")).toBe("Uploaded source artwork");
  });
});
