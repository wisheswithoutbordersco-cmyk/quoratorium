...
export function createClerkAppMiddleware() {
  // Use the same production publishable key as ClerkProvider in the browser.
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    const handleError = (error: unknown) => {
      ...
      const isProxyRequest =
        req.path === "/__clerk" || req.path.startsWith("/__clerk/");
      ...
      if (isProxyRequest) {
        ...
        if (!res.headersSent) {
          res
            .status(502)
            .json({ error: "Owner sign-in is temporarily unavailable." });
        }
        return;
      }
