...
describe("Clerk application middleware", () => {
  it("configures Clerk's own same-origin frontend API proxy on /__clerk", () => {
    vi.mocked(clerkMiddleware).mockReturnValue(((req: any, _res: any, next: any) => {
  ...
  it("returns a controlled 502 for proxy failure without continuing into private routes", () => {
    vi.mocked(clerkMiddleware).mockReturnValue(((_req: any, _res: any, next: any) => {
      next(new Error("proxy unavailable"));
    }) as any);
    const middleware = createClerkAppMiddleware();
    ...
    expect(res.json).toHaveBeenCalledWith({ error: "Owner sign-in is temporarily unavailable." });
    expect(next).not.toHaveBeenCalled();
  });

  it("clears stale cookies and continues only as unauthenticated", () => {
    vi.mocked(clerkMiddleware).mockReturnValue(((_req: any, _res: any, next: any) => {
      next(new Error("JWKS signing key mismatch"));
    }) as any);
    const middleware = createClerkAppMiddleware();
    const req: any = { path: "/api/trpc" };
    const res = response();
