...
const builderDefs = [
  {
    ...
    description:
      "Routes tasks to appropriate workers and manages execution plans",
  },
  {
    id: "builder",
    name: "Builder",
...
export default function Builders() {
  const { data: recentActivity, isLoading } =
    trpc.projects.getRecentActivity.useQuery({ limit: 50 });
  const {
    data: poolHealth,
    isLoading: poolHealthLoading,
    isError: poolHealthError,
  } = trpc.observability.poolHealth.useQuery(undefined, {
    retry: 1,
  ...
  const poolHealthDisplay =
    poolHealthError || !poolHealth
      ? {
          label: "Unavailable",
          detail: "Pool-health data could not be loaded.",
      ...
      : poolHealthLoading
        ? {
            label: "Checking",
            detail: "Loading provider and telemetry status.",
  ...
  const getAgentStats = (agentId: string) => {
    if (!recentActivity)
      return { tasksCompleted: 0, lastTask: "Awaiting tasks" };
    ...
    const completedEvents = agentEvents.filter(
      e => e.eventType.includes("complete") || e.eventType.includes("response")
    );
  ...
  const isAgentActive = (agentId: string) => {
    const storeAgent = agents.find(
      a => a.id === agentId || a.type.toLowerCase().includes(agentId)
    );
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between mb-8">
            ...
            <div
              className="flex items-center gap-2 px-3 py-2 rounded-lg surface-elevated border border-border"
              title={poolHealthDisplay.detail}
            >
              <poolHealthDisplay.Icon
                size={12}
                className={`${poolHealthDisplay.className} ${poolHealthLoading ? "animate-spin" : ""}`}
              />
              <span className="text-[10px] text-muted-foreground/60">
                Pool Health: {poolHealthDisplay.label}
              </span>
            </div>
          </div>
                    ...
                    <div className="p-4">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex items-center gap-1.5">
                          <div
                            className={`w-2 h-2 rounded-full ${active ? "bg-[#10B981] animate-pulse" : "bg-muted-foreground/20"}`}
                          />
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-3">
                        <div className="text-center">
                          <p className="text-[14px] font-display text-foreground">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {builder.capabilities.map(cap => (
                          <span
                            key={cap}
                            className="px-2 py-0.5 rounded text-[9px] surface-overlay border border-border text-muted-foreground/50"
                          >
                            {cap}
                          </span>
                        ))}
